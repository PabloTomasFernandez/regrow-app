...
if not admin_mode:
    st.caption("Modo solo lectura. Solo ves los proyectos donde estás asignado.")
...
with Session(engine) as session:
    ...
    projects = filter_visible_projects(viewer, all_projects)  # type: ignore[assignment]
...
for project in projects:
    ...
    with st.expander(title):
        ...
        if admin_mode and proj_assignments and project.id is not None:
                        ...
                        if c1.button("Confirmar cambio", key=f"confirm_{a.id}"):
                            ...
                            with Session(engine) as session:
                                ...
                                session.exec(
                                    ...
                                    .where(TaskDB.project_id == pid)  # type: ignore[arg-type]
                                    .where(TaskDB.assigned_to == old_mid)  # type: ignore[arg-type]
        ...
        if project.status == ProjectStatus.active:
            ...
            if admin_mode:
                ...
                if btn_cols[1].button(
                    "Completar proyecto", key=f"complete_{project.id}"
                ):
            ...
            if admin_mode:
                ...
                if st.button("Activar", key=f"activate_{project.id}"):
                    if project.id is None:
                        ...
                        tasks = generate_base_tasks(
                            project.id, onboarding_date, tmpl_list
                        )
                        tasks.extend(
                            generate_checkups(
                                project.id,
        ...
        if admin_mode and project.status in (ProjectStatus.draft, ProjectStatus.paused):
            st.divider()
