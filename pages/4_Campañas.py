...
from regrow.domain.models import (
    CampaignType,
    CopyStatus,
    ProjectStatus,
    TaskStatus,
)
from regrow.ui.permissions import (
    filter_visible_projects,
    is_admin,
    viewer_roles_in_project,
)
...
with Session(engine) as session:
    ...
    active_projects = filter_visible_projects(viewer, all_active_projects)  # type: ignore[assignment]
...
if not admin_mode and not active_projects:
...
viewer_roles = viewer_roles_in_project(viewer, project_id)
    ...
    with st.form("create_campaign", clear_on_submit=True):
        ...
        if submitted:
            if not industry.strip() or not country.strip():
                ...
                st.error("Nombre del evento obligatorio para tipo event.")
            else:
                with Session(engine) as session:
...
for campaign in campaigns:
    ...
    with st.expander(header):
        if admin_mode or "account_manager" in viewer_roles:  # pyright: ignore[reportOptionalCall]
        ...
        if admin_mode or "copy" in viewer_roles:
